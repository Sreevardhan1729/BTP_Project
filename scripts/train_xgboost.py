...
logger = get_logger("fdof.train_xgboost")
...
def main():
    parser = argparse.ArgumentParser(description="Step 6: Train & Evaluate XGBoost on selected features")
    parser.add_argument("--config", type=str, default="configs/config-xgboost.yaml",
                        help="Path to XGBoost config YAML")
    ...
    model_cfg = ModelConfig(
        type=str(model.get("type", "xgboost")),
        ...
        grid_max_depth=tuple(model.get("grid", {}).get("max_depth", [3, 5, 7])),
